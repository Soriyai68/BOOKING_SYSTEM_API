...
showtimeSchema.pre("save", async function (next) {
        ...
        if (this.movie_id && this.start_time) {
            try {
                ...
                if (movie && movie.duration_minutes) {
                    ...
                    const startDateTime = new Date(showDate.getFullYear(), showDate.getMonth(), showDate.getDate(), startHours, startMinutes);
                    const totalDuration = movie.duration_minutes;
                    const endDateTime = new Date(startDateTime.getTime() + totalDuration * 60000);
                    this.end_time = `${String(endDateTime.getHours()).padStart(2, "0")}:${String(endDateTime.getMinutes()).padStart(2, "0")}`;
                }
            } catch (error) {
                return next(new Error(`Failed to calculate end time: ${error.message}`));
            }
        }
    }

    if (!this.end_time) {
        return next(new Error("Showtime end_time is required but was not provided and could not be calculated."));
    }

    if (this.isNew || this.isModified("start_time") || this.isModified("show_date")) {
        const showDate = new Date(this.show_date);
        const [startHours, startMinutes] = this.start_time.split(":").map(Number);
        const startDateTime = new Date(showDate.getFullYear(), showDate.getMonth(), showDate.getDate(), startHours, startMinutes);
        if (startDateTime < new Date()) {
            return next(new Error("Showtime start date and time cannot be in the past."));
        }
    }

    if (this.isNew || this.isModified("start_time") || this.isModified("end_time") || this.isModified("show_date")) {
        const overlapping = await this.constructor.findOverlappingShowtimes(this.hall_id, this.show_date, this.start_time, this.end_time, this._id);
        if (overlapping.length > 0) {
            return next(new Error("Showtime overlaps with an existing showtime in the same hall."));
        }
    }
    ...
    if (this.isModified('status') && this.status === 'cancelled') {
        ...
        try {
            logger.info(`Showtime ${this._id} cancelled. Cancelling associated bookings.`);
            ...
            if (bookingsToCancel.length > 0) {
                ...
                logger.info(`Cancelled ${bookingsToCancel.length} bookings for showtime ${this._id}.`);
            } else {
                logger.info(`No active bookings to cancel for showtime ${this._id}.`);
            }
        } catch (error) {
            logger.error(`Error cancelling bookings for showtime ${this._id}: ${error.message}`);
        }
    }
