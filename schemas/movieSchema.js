...
const createMovieSchema = Joi.object({
  ...
  duration_minutes: Joi.number().integer().min(1).max(500).required().messages({
  ...
  poster_url: Joi.string().trim().uri().allow(null, "").optional().messages({
    "string.uri": "Poster URL must be a valid URL",
  }),

  trailer_url: Joi.string().trim().uri().allow(null, "").optional().messages({
    "string.uri": "Trailer URL must be a valid URL",
  }),

  director: Joi.string().trim().max(100).allow("").default("").messages({
...
const updateMovieSchema = Joi.object({
  ...
  duration_minutes: Joi.number().integer().min(1).max(500).messages({
    "number.base": "Duration must be a number",
    "number.integer": "Duration must be an integer",
    "number.min": "Duration must be at least 1 minute",
    "number.max": "Duration cannot exceed 500 minutes",
  ...
  poster_url: Joi.string().trim().uri().allow(null, "").messages({
    "string.uri": "Poster URL must be a valid URL",
  }),

  trailer_url: Joi.string().trim().uri().allow(null, "").messages({
    "string.uri": "Trailer URL must be a valid URL",
  }),

  director: Joi.string().trim().max(100).allow("").messages({
    "string.max": "Director name cannot exceed 100 characters",
  }),

  producers: Joi.array().items(Joi.string().trim().min(1).max(100)).messages({
    "string.min": "Producer name must be at least 1 character",
    "string.max": "Producer name cannot exceed 100 characters",
  }),
...
const paginationSchema = Joi.object({
  ...
  sortBy: Joi.string()
    ...
    .messages({
      "any.only":
        "SortBy must be one of: title, release_date, rating, duration_minutes, createdAt, updatedAt, deletedAt",
    }),
