...
const HALL_SCREEN_TYPES = ["2d", "standard", "imax", "3d", "4dx", "vip"];
const HALL_STATUSES = ["active", "maintenance", "closed", "renovation"];
const HALL_FEATURES = [
  "dolby_atmos",
  "surround_sound",
  "premium_seating",
...
const createHallSchema = Joi.object({
  ...
  screen_type: Joi.string()
    .valid(...HALL_SCREEN_TYPES)
    .default("standard")
    .messages({
      "any.only": `Screen type must be one of: ${HALL_SCREEN_TYPES.join(", ")}`,
    }),
  ...
  status: Joi.string()
    ...
    .default("active")
    .messages({
      "any.only": `Status must be one of: ${HALL_STATUSES.join(", ")}`,
    }),

  features: Joi.array()
    .items(Joi.string().valid(...HALL_FEATURES))
    .unique()
    .default([])
    .messages({
      "array.unique": "Duplicate features are not allowed",
      "any.only": `Features must be one of: ${HALL_FEATURES.join(", ")}`,
    }),
...
const updateHallSchema = Joi.object({
  ...
  theater_id: Joi.string().trim().allow(null, ""),
...
const hallIdParamSchema = Joi.object({
  id: Joi.string()
    .pattern(/^[0-9a-fA-F]{24}$/)
    .required()
    .messages({
...
const hallScreenTypeParamSchema = Joi.object({
  type: Joi.string()
    .valid(...HALL_SCREEN_TYPES)
    .required()
    .messages({
...
const theaterIdParamSchema = Joi.object({
  theaterId: Joi.string().trim().required().messages({
    "any.required": "Theater ID is required",
  }),
});

// Query validation schemas
const getAllHallsQuerySchema = Joi.object({
...
const advancedSearchSchema = Joi.object({
  ...
  fields: Joi.array()
    .items(
      Joi.string().valid("hall_name", "screen_type", "theater_id", "notes")
    )
    .optional()
    .default(["hall_name"]),
  page: Joi.number().integer().min(1).default(1),
...
const batchDeleteSchema = Joi.object({
  ids: Joi.array()
    ...
    .required()
    .messages({
      "array.min": "At least one hall ID is required",
      "array.max": "Cannot delete more than 100 halls at once",
      "any.required": "Hall IDs array is required",
    }),
...
const batchUpdateStatusSchema = Joi.object({
  ids: Joi.array()
    ...
    .messages({
      "array.min": "At least one hall ID is required",
      "array.max": "Cannot update more than 100 halls at once",
      "any.required": "Hall IDs array is required",
    }),
  status: Joi.string()
    .valid(...HALL_STATUSES)
...
// Theater halls query schema
const theaterHallsQuerySchema = Joi.object({
...
const analyticsQuerySchema = Joi.object({
  ...
  dateTo: Joi.date().iso().min(Joi.ref("dateFrom")).optional(),
  groupBy: Joi.string()
    .valid("theater_id", "screen_type", "status", "month", "week")
    .default("screen_type"),
}).messages({
  "date.min": "dateTo must be greater than or equal to dateFrom",
});
