...
const createTheaterSchema = Joi.object({
  ...
  total_halls: Joi.number().integer().min(0).max(50).default(0).messages({
    "number.base": "Total halls must be a number",
    "number.integer": "Total halls must be an integer",
    "number.min": "Total halls cannot be negative",
    "number.max": "Total halls cannot exceed 50",
  }),

  total_capacity: Joi.number().integer().min(0).default(0).messages({
  ...
  notes: Joi.string().trim().max(1000).allow("").default("").messages({
...
const updateTheaterSchema = Joi.object({
  ...
  province: Joi.string().trim().min(1).max(100).messages({
    "string.min": "Province must be at least 1 character",
    "string.max": "Province cannot exceed 100 characters",
  }),
  ...
  total_halls: Joi.number().integer().min(0).max(50).messages({
    "number.base": "Total halls must be a number",
    "number.integer": "Total halls must be an integer",
    "number.min": "Total halls cannot be negative",
    "number.max": "Total halls cannot exceed 50",
  ...
  total_capacity: Joi.number().integer().min(0).messages({
    "number.base": "Total capacity must be a number",
    "number.integer": "Total capacity must be an integer",
    "number.min": "Total capacity cannot be negative",
  }),
...
const getAllTheatersQuerySchema = Joi.object({
  ...
  minHalls: Joi.number().integer().min(0).optional(),
  maxHalls: Joi.number().integer().min(Joi.ref("minHalls")).optional(),
  minCapacity: Joi.number().integer().min(0).optional(),
  maxCapacity: Joi.number().integer().min(Joi.ref("minCapacity")).optional(),
  hasFeatures: Joi.array()
    .items(Joi.string().valid(...THEATER_FEATURES))
    .unique()
    .optional(),
  dateFrom: Joi.date().iso().optional(),
  dateTo: Joi.date().iso().min(Joi.ref("dateFrom")).optional(),
  nearLocation: Joi.string()
    .pattern(/^-?\d+\.?\d*,-?\d+\.?\d*$/)
    .optional()
    .messages({
      "string.pattern.base":
        'Near location format must be "longitude,latitude"',
    }),
  maxDistance: Joi.number().positive().default(10000),
}).messages({
  "number.min": "maxHalls must be greater than or equal to minHalls",
...
const addHallSchema = Joi.object({
  hall_id: Joi.string()
    .pattern(/^[0-9a-fA-F]{24}$/)
    .required()
    .messages({
...
const removeHallSchema = Joi.object({
  hall_id: Joi.string()
    .pattern(/^[0-9a-fA-F]{24}$/)
    .required()
    .messages({
...
const advancedSearchSchema = Joi.object({
  ...
  "number.min":
    "maxHalls/maxCapacity must be greater than or equal to minHalls/minCapacity",
  "date.min": "dateTo must be greater than or equal to dateFrom",
});
...
const locationTheatersQuerySchema = Joi.object({
  ...
  includeHalls: Joi.string().valid("true", "false").default("false"),
  sortBy: Joi.string()
    .valid("name", "total_halls", "total_capacity", "createdAt")
    .default("name"),
  sortOrder: Joi.string().valid("asc", "desc").default("asc"),
});
...
const analyticsQuerySchema = Joi.object({
  ...
  dateTo: Joi.date().iso().min(Joi.ref("dateFrom")).optional(),
  groupBy: Joi.string()
    .valid("city", "province", "status", "month", "week")
    .default("province"),
}).messages({
  "date.min": "dateTo must be greater than or equal to dateFrom",
});
