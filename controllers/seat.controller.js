...
class SeatController {
    ...
    try {
      ...
      if (!hall_id || !row || !range?.start || !range?.end) {
        return res.status(400).json({
          success: false,
          message:
            "Missing required fields: hall_id, row, and range are required.",
        });
      }
      ...
      if (seatCount > 15) {
        return res.status(400).json({
          message: `Seat range is limited to 15 per request. You tried to create ${seatCount}.`,
        });
      }
      // const seatNumbers = [];
      ...
      if (existingSeats.length > 0) {
        return res.status(409).json({
          ...
          existingSeats: existingSeats.map((s) => `${s.row}${s.seat_number}`),
        });
      }
      ...
      if (createdSeats.length > 0) {
        try {
          ...
          logger.info(
            `Updated total_seats for hall ${hall_id} after bulk creation.`,
          );
        } catch (hallError) {
          logger.error(
      ...
      logger.info(
        `Bulk created ${createdSeats.length} seats for hall ${hall_id}.`,
      );
      ...
      return res.status(500).json({
        success: false,
        message:
          "An unexpected server error occurred during bulk seat creation.",
        error: error.message,
      });
    }
  }
  ...
  static async bulkUpdateSeats(req, res) {
    try {
      ...
      if (
        !seatUpdates ||
        !Array.isArray(seatUpdates) ||
        seatUpdates.length === 0
      ) {
        return res.status(400).json({
          success: false,
          message:
            "Request body must contain a non-empty array of seat updates.",
        });
      }
      const results = [];
  ...
  static async bulkDuplicateSeats(req, res) {
    try {
      ...
      const invalidIds = seat_ids.filter(
      ...
      const existingKeys = new Set(
        existingTargetSeats.map(
          (s) =>
            `${s.row}-${Array.isArray(s.seat_number) ? s.seat_number.join(",") : s.seat_number}`,
        ),
      );
      ...
      try {
        ...
        logger.info(
          `Updated total_seats for target hall ${target_hall_id} after bulk duplication.`,
        );
      } catch (hallError) {
        logger.error(
          `Failed to update total_seats for hall ${target_hall_id}: ${hallError.message}`,
        );
      }

      logger.info(
        `Bulk duplicated ${createdSeats.length} seats to hall ${target_hall_id}.`,
      );
      ...
      return res.status(500).json({
        success: false,
        message:
          "An unexpected server error occurred during bulk seat duplication.",
        error: error.message,
      });
    }
  }
    ...
    try {
      ...
      if (req.user?.role !== Role.ADMIN && req.user?.role !== Role.SUPERADMIN) {
        return res.status(403).json({
          success: false,
          message:
            "Forbidden: Only Admins or SuperAdmins can permanently delete seats.",
        });
      }
      ...
      const seatMap = new Map(
        seatsToDelete.map((seat) => [seat._id.toString(), seat]),
      );
      ...
      if (hallIdsToUpdate.size > 0) {
        logger.info(
          `Updating total_seats for halls: ${[...hallIdsToUpdate].join(", ")}`,
        );
        for (const hallId of hallIdsToUpdate) {
          try {
            ...
            logger.error(
              `Failed to update total_seats for hall ${hallId}: ${hallError.message}`,
            );
      ...
      if (deletedSeatIds.length > 0) {
        const { deletedCount: sbDeletedCount } = await SeatBooking.deleteMany({
          seatId: { $in: deletedSeatIds },
        });
        logger.info(
          `Deleted ${sbDeletedCount} SeatBooking records for ${deletedSeatIds.length} permanently deleted seats.`,
        );
      }
