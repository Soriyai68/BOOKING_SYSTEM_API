...
class BookingTicketController {
    ...
    static async getAll(req, res) {
        try {
            const { page = 1, limit = 10, sortBy = 'issuedAt', sortOrder = 'desc', ...filters } = req.query;
            ...
            if (filters.seatId && mongoose.Types.ObjectId.isValid(filters.seatId)) {
                matchQuery['seat.seat_id'] = new mongoose.Types.ObjectId(filters.seatId);
            }
            ...
            // 3. Match for the search filter, which requires looked-up fields
            if (filters.search) {
                const searchQuery = {
                    $or: [
                        { ticket_code: { $regex: filters.search, $options: 'i' } },
                        { 'seat.seat_number': { $regex: filters.search, $options: 'i' } },
            ...
            res.status(500).json({ success: false, message: 'Failed to retrieve booking tickets' });
        }
    }
    ...
    static async getById(req, res) {
        try {
            ...
            res.status(500).json({ success: false, message: 'Failed to retrieve booking ticket' });
        }
    }
    ...
    static async create(req, res) {
        try {
            ...
            res.status(500).json({ success: false, message: 'Failed to create booking ticket' });
        }
    }
    ...
    static async update(req, res) {
        try {
            ...
            const ticket = await BookingTicket.findByIdAndUpdate(id, { $set: updateData }, { new: true, runValidators: true });
            ...
            res.status(500).json({ success: false, message: 'Failed to update booking ticket' });
        }
    }
    ...
    static async delete(req, res) {
        try {
            ...
            res
                .status(200)
                .json({ success: true, message: "Booking ticket deleted successfully" });
        } catch (error) {
